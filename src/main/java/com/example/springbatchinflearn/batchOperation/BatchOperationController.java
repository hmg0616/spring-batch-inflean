...
/*
        ...
        for(Iterator<String> iterator = jobRegistry.getJobNames().iterator(); iterator.hasNext();){
            ...
            jobOperator.start(job.getName(), "id=" + jobInfo.getId()); // job이름, jobParameters
        }
*/
        System.out.println("jobName : " + "jobOperationConfigurationJob");
        jobOperator.start("jobOperationConfigurationJob", "id=" + jobInfo.getId()); // job이름, jobParameters
...
*/
        ...
        Set<JobExecution> runningJobExecutions = jobExplorer.findRunningJobExecutions("jobOperationConfigurationJob");
        JobExecution jobExecution = runningJobExecutions.iterator().next();
    ...
    public String restart() throws NoSuchJobException, NoSuchJobExecutionException, JobRestartException, JobParametersInvalidException, JobInstanceAlreadyCompleteException {
/*
        for(Iterator<String> iterator = jobRegistry.getJobNames().iterator(); iterator.hasNext();){
            SimpleJob job = (SimpleJob)jobRegistry.getJob(iterator.next());
            System.out.println("jobName : " + job.getName());
...
*/
        ...
        JobInstance lastJobInstance = jobExplorer.getLastJobInstance("jobOperationConfigurationJob");
        JobExecution lastJobExecution = jobExplorer.getLastJobExecution(lastJobInstance);
